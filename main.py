...
def load_data(test_data):
    ...
    if test_data:
        ...
        select = "train"
    select_df = df[df["split"] == select]

    X = []
    X_file = select_df["file"]
...
if __name__ == "__main__":
    X_train, y_train = load_data(test_data=False)
    X_test, y_test = load_data(test_data=True)
    ...
    train_model()
    print(evaluate_model(X_train_tensor, y_train_tensor), evaluate_model(X_test_tensor, y_test_tensor))
